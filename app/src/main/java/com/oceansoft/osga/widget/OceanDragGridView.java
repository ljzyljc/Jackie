...
import com.oceansoft.osga.moudle.home.adapter.AbsDragAdapter;
 ...
 * <p/>
 * 使用方式：
 * 1、直接可以在XML文件中引入，然后通过findViewById获取。
 * 2、对外公布OnChanageListener 实现交换数据。可以自定义哪些数据可以交换，哪些不可以交换等操作。
 * 实现类的核心代码：
 * <p/>
 * // 这里的交换数据源的数据
 * if (from < to) {
 * for (int i = from; i < to; i++) {
 * Collections.swap(gridItems, i, i + 1);
 * ...
 * }
 * } else if (from > to) {
 * for (int i = from; i > to; i--) {
 ...
 * }
 * <p/>
 * gridItems.set(to, temp);
 * <p/>
 * // 设置新到的item隐藏，不用调用notifyDataSetChanged来刷新界面，因为setItemHide方法里面调用了
 * gridAdapter.setItemHide(to);
 * return true;
 * <p/>
 * =========================================================
 * 3、使用的Adapter必须继承AbsDragAdapter，具体实现方式见AbsDragAdapter类的注释
 * =========================================================
 * <p/>
 * 4、将dispatchTouchEvent方法中switch..case中MotionEvent.ACTION_DOWN:里面的
 * <p/>
 * HomeGridAdapter adapter = (HomeGridAdapter) getAdapter();
 * if (adapter != null) {
 * if (mDragPosition == 0 || mDragPosition >= adapter.getUnMoveIndex()) {
 * return super.dispatchTouchEvent(ev);
 * }
 * }
 * 这些代码去掉，这是当前项目的需求。要求第一项和最后几项不可以拖动。
 */
public class OceanDragGridView<T> extends GridView {
    ...
    public OceanDragGridView(Context context) {
        this(context, null);
    }

    public OceanDragGridView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public OceanDragGridView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void onSwapItem(int moveX, int moveY) {
        ...
        if (tempPosition != mDragPosition && tempPosition != AdapterView.INVALID_POSITION) {
            if (onChanageListener != null) {
                if (!onChanageListener.onChange(mDragPosition, tempPosition)) return;
            }
    ...
    public interface OnChanageListener {
         ...
         */
        public boolean onChange(int form, int to);
